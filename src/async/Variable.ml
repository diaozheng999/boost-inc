...
let read_and_update_value value =
  match value.last_value with
    ...
    | None -> value.read () [@bs] |> Js.Promise.then_ (fun (resolved_value, addr) ->
...
let execute_current_observers ~variable ~value =
  Propagate.Task_queue.insert (fun [@bs] () -> fire_static_observers ~variable ~value);
  Linked_list.for_each_node variable.observers ~f:(fun node ->
    match node.value.window with
      | (t1, _) when Time.isSplicedOut t1 -> Linked_list.remove_node variable.observers node
      | window -> Propagate.Task_queue.insert ~window (Bs_interop.bind node.value.next value)
  )
