...
let execute ~loop =
  let f () =
    ...
    loop () [@bs] |> Bs_interop.exec_unit_finally (fun () -> state.is_propagating <- false)
  in
  Mutex.lock lock f
...
let until ~time =
  let rec loop = fun [@bs] () ->
    match Task_queue.find_min () with
      ...
      | Some { task; window = None } -> task () [@bs] |> Bs_interop.promise_then_0 loop
...
let when_not_propagating f =
  if state.is_propagating then
    Mutex.acquire_readonly_access lock |> Bs_interop.unstable_promise_then_unit_exec f
  else
    Js.Promise.resolve ((Bs_interop.arity0 f) () [@bs])
